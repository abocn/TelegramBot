...
<Cards className="grid-cols-1">
    <Card>
        <div className="flex flex-row items-center gap-1 text-red-500 -mb-2">
            <TbAlertCircle /> <div>Important</div>
        </div>
        You will only need all of them if you are not running it dockerized. Read ["Running with Docker"](/self-hosting/docker) for more information.
    </Card>
</Cards>
...
<Cards>
    <Card title="Install in Docker" href="/self-hosting/docker" icon={<SiDocker className="size-4" />} />
    <Card title="Install Locally" href="/self-hosting/local" icon={<Laptop className="size-4" />} />
</Cards>
