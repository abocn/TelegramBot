...
export default (bot: Telegraf<Context>, db: NodePgDatabase<typeof schema>) => {
  bot.command("mlp", spamwatchMiddleware, async (ctx: Context & { message: { text: string } }) => {
    ...
    try {
      ...
      await sendReply(ctx, Strings.ponyApi.helpDesc, reply_to_message_id);

      await trackCommand(db, ctx, 'mlp', true, undefined, startTime);
    } catch (error) {
      await trackCommand(db, ctx, 'mlp', false, error.message, startTime);
      throw error;
    }
  });
    ...
    try {
      ...
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          await sendPhoto(ctx, character.image[0], result, reply_to_message_id);
        } else {
          await sendReply(ctx, Strings.ponyApi.noCharFound, reply_to_message_id);
        }
      } catch (error: any) {
        const message = Strings.ponyApi.apiErr.replace('{error}', error.message || 'Unknown error');
        await sendReply(ctx, message, reply_to_message_id);
        throw error;
      }

      await trackCommand(db, ctx, 'mlpchar', true, undefined, startTime);
    } catch (error) {
      await trackCommand(db, ctx, 'mlpchar', false, error.message, startTime);
      throw error;
    }
  });
  ...
  bot.command("mlpep", spamwatchMiddleware, async (ctx: Context & { message: { text: string } }) => {
    ...
    try {
      ...
      if (Number(userInput) > 10000) {
        await ctx.reply(Strings.mlpInvalidEpisode, {
          parse_mode: 'Markdown',
          ...(reply_to_message_id ? { reply_parameters: { message_id: reply_to_message_id } } : {})
        });
        return;
      ...
      try {
        ...
        if (episodeArray.length > 0) {
          ...
          await ctx.replyWithPhoto(episodeArray[0].image, {
            caption: `${result}`,
            parse_mode: 'Markdown',
            ...(reply_to_message_id ? { reply_parameters: { message_id: reply_to_message_id } } : {})
          });
        } else {
          await ctx.reply(Strings.ponyApi.noEpisodeFound, {
            parse_mode: 'Markdown',
            ...(reply_to_message_id ? { reply_parameters: { message_id: reply_to_message_id } } : {})
          });
        };
      ...
      await trackCommand(db, ctx, 'mlpep', true, undefined, startTime);
    } catch (error) {
      await trackCommand(db, ctx, 'mlpep', false, error.message, startTime);
      throw error;
    }
  });
  ...
  bot.command("mlpcomic", spamwatchMiddleware, async (ctx: Context & { message: { text: string } }) => {
    ...
    try {
      ...
      if (/[^a-zA-Z0-9\s]/.test(userInput) || userInput.length > 30) {
        await ctx.reply(Strings.ponyApi.noComicName, {
          parse_mode: 'Markdown',
          ...(reply_to_message_id ? { reply_parameters: { message_id: reply_to_message_id } } : {})
        });
        return;
      ...
      try {
        ...
        if (comicArray.length > 0) {
          ...
          await ctx.replyWithPhoto(comicArray[0].image, {
            caption: `${result}`,
            parse_mode: 'Markdown',
            ...(reply_to_message_id ? { reply_parameters: { message_id: reply_to_message_id } } : {})
          });
        } else {
          await ctx.reply(Strings.ponyApi.noComicFound, {
            parse_mode: 'Markdown',
            ...(reply_to_message_id ? { reply_parameters: { message_id: reply_to_message_id } } : {})
          });
        };
      ...
      await trackCommand(db, ctx, 'mlpcomic', true, undefined, startTime);
    } catch (error) {
      await trackCommand(db, ctx, 'mlpcomic', false, error.message, startTime);
      throw error;
    }
  });
};
